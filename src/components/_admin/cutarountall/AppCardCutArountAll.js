...
export default function AppCardCutArountAll(props) {
  ...
  //   `${process.env.REACT_APP_WEB_BACKEND}/getByCutArountID/${props.props.cut_arount_id}`
  // );
  ...
  // }, [props.props.cut_arount_province]);
  const onClickCard = async (req, res) => {
    const getOrderDetail = await axios.get(
      `${process.env.REACT_APP_WEB_BACKEND}/getByCutArountID/${req}`
    );
    const filtereds = [];
  ...
  return (
    <>
      <RootStyle onClick={async () => await onClickCard(props.props.cut_arount_id)}>
        {/* <IconWrapperStyle> */}
        {/* <CheckCircleOutlined style={{ fontSize: '28px' }} /> */}
        {/* </IconWrapperStyle> */}
        <Typography variant="subtitle2" sx={{ opacity: 0.72 }}>
          รหัสการตัดรอบ : <br />
          {props.props.cut_arount_id}
        </Typography>
        <Typography variant="h3">{props.props.cut_arount_province}</Typography>
        <Typography variant="subtitle2" sx={{ opacity: 0.72 }}>
        ...
        <Typography variant="subtitle2" sx={{ opacity: 0.72 }}>
          {' '}
          {props.props.cut_arount_date
            ? `${dayjs(props.props.cut_arount_first_date)
                .locale('th')
                .format('DD MMMM YYYY')} ถึง ${dayjs(props.props.cut_arount_last_date)
                .locale('th')
                .format('DD MMMM YYYY')}`
            : null}
        </Typography>
      ...
      >
        <div ref={(el) => (componentRef = el)}>
          ...
          <DialogTitle> จังหวัด: {props.props.cut_arount_province}</DialogTitle>
