...
import AppCardCutArountPartner from '../../../../components/_admin/cutarount/AppCardCutArountPartner';
// ----------------------------------------------------------------------
function AdminConfirmExpressApp() {
  ...
  useEffect(async () => {
    ...
    const filterOrderStatus = addPartnerLevel.filter(
      (f) => f.order_partner_status === 'รอจัดส่ง' && f.odd_status === 'ตัดรอบการจัดส่งแล้ว'
    );
    setValueDetail(filterOrderStatus);
    const filtereds = [];
    filterOrderStatus.forEach((item, index) => {
      const idx = filtereds.findIndex((value) => value.odd_cutarount_id === item.odd_cutarount_id);
      if (idx === -1) {
        filtereds.push(item);
      }
    });
  ...
  return (
    <Page title="รวมยอดสินค้า | admin NBA-Express">
      <Container maxWidth="xl">
        <Box sx={{ pb: 5 }}>
          <Typography variant="h4">รายการสินค้าเเต่ละ เขต / ภาค ที่ถูกตัดรอบแล้ว </Typography>
        </Box>
        <br />
        <Grid container spacing={3}>
          {Order?.map((item) => (
            ...
            <Grid item xs={12} sm={6} md={3}>
              <AppCardCutArountPartner props={item} valueDetail={valueDetail} />
